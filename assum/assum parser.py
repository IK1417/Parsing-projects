    ...
    for line in soup.find_all(class_='vc_row'):
        for product in line.find_all(class_='wpb_column'):
            ...
            for proposal in product_soup.find_all(class_='assum_modification__product_block'):
                ...
                IE_XML_ID += 1
                try:
                    proposal_data['IC_GROUP_LIST'] = [i.text for i in
                                                  product_soup.find(class_='breadcrumbs text-small').find_all('li')[
                                                  1:-1]] + [product_soup.find(class_='page-title-head').text] + [product_soup.find(class_='page-title-head').text]
                    IC_GROUP_COUNT = max(len(item_data['IC_GROUP_LIST']), IC_GROUP_COUNT)
                except Exception as ex:
                    proposal_data['IC_GROUP_LIST'] = []
                proposal_data['CV_CURRENCY_1'] = 'RUB'
                proposal_data['IE_PREVIEW_TEXT'] = ''
                proposal_data['IE_PREVIEW_TEXT_TYPE'] = 'html'
                proposal_data['IE_DETAIL_TEXT_TYPE'] = 'html'
                try:
                    proposal_data['IE_PREVIEW_PICTURE'] = product.find('img')['src']
                except Exception:
                    proposal_data['IE_PREVIEW_PICTURE'] = ''
                try:
                    proposal_data['IE_DETAIL_PICTURE'] = proposal_data['IE_PREVIEW_PICTURE']
                except Exception:
                    proposal_data['IE_DETAIL_PICTURE'] = ''
                ...
                try:
                    proposal_data['IE_DETAIL_TEXT'] = proposal_soup.find(class_='woocommerce-product-details__short-description').find('p').text
                except Exception:
                    proposal_data['IE_DETAIL_TEXT'] = ''
                IP_PROP_LIST_PROPOSAL["Производитель"] = "IP_PROP74"
                IP_PROP_LIST_PROPOSAL["Страна"] = "IP_PROP265"
                proposal_data["IP_PROP74"] = 'ООО «АССУМ»'
                proposal_data["IP_PROP265"] = 'Россия'
                IP_PROP_LIST_PROPOSAL["Привязка к бренду"] = "IP_PROP61"
                proposal_data["IP_PROP61"] = 296
                try:
                    ...
                    pass
                all_data.append(proposal_data)
            #all_data.append(item_data)
            print(product_soup.find(class_='page-title-head').text)
            print(url_product)
            print('\n')

    with open(f"assum data/{name}.csv", "w", encoding='utf-8', newline='') as file:
        writer = csv.writer(file, delimiter=';')
        header_table = tuple(
            ['IE_XML_ID', 'IE_NAME', 'IE_PREVIEW_PICTURE', 'IE_PREVIEW_TEXT', 'IE_PREVIEW_TEXT_TYPE', 'IE_CODE',
             'IE_DETAIL_TEXT_TYPE', 'IE_DETAIL_PICTURE', 'IE_DETAIL_TEXT'] + list(IP_PROP_LIST_PROPOSAL.values()) + [
                f"IC_GROUP{u}" for u in range(IC_GROUP_COUNT)] + ['CV_PRICE_1', 'CV_CURRENCY_1'])
        writer.writerow(header_table)
    with open(f"assum data/{name}.csv", "a", encoding='utf-8', newline='') as file:
